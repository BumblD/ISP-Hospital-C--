...
namespace HospitalBackEnd.Controllers
{
    public class VartotojasController : Controller
    {
        ...
        public IActionResult CreateDoctor(VartotojasViewModel user)
        {
            ...
            if (user.Specializacijos_Kodas == null)
            {
        ...
        [HttpPut]
        public IActionResult AssignDoctor([FromRoute] string id, [FromBody] VartotojasViewModel doctor)
        {
            Db.Connection.Open();
            var cmd = Db.Connection.CreateCommand() as MySqlCommand;
            ...
            cmd.Parameters.AddWithValue("@doctor", doctor.Priskirtas);
            cmd.Parameters.AddWithValue("@id", id);
            cmd.ExecuteNonQuery();
